...
| Site Links, Add New Link, existing site table + edit form | Missing, two sub-features narrowing | Two separate site-related features, must not be conflated: (1) **Crawl Links** — `#addNewCrawlLink` → `#crawlLinkSite1` input → `#crawlSaveButtonSite1` (re-sent as a correctly matched row-1 triple; the earlier 1/2 mismatch was a copy artifact). Each row appears to save independently via its own numbered button. Still open: is `crawlLinkSite{n}` a fixed small set of pre-rendered numbered slots, or does clicking "Add New Link" append a new numbered slot each time? Need to know before building — the id-numbering pattern is different enough from the name-variations pattern (generic shared classes, no numbering) that it shouldn't be assumed to work the same way. (2) **Add New Site form** (Site Name, Site Type dropdown, City, Country dropdown with ~150 options, Province — confirmed a plain text input, not a dropdown, Zip, Phone, Fax, Email, Status dropdown, one shared `#saveSiteInfoButton`). Address Line 1/2 confirmed correct and unambiguous: `site.address1` and `site.address2` both exist as separate fields (the earlier flagged concern is resolved). Still unconfirmed: does "Add New Site" open inline or in a separate window (like Industries/Verticals)? |
